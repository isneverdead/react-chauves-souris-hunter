...
function App() {
  ...
  return (
    <div className="App relative flex h-screen bg-gray-200">
      {gameState === 'idle' ? (
        <Idle handleGameState={(value) => setGameState(value)} isLoading={loading} />
      ) : null}
      {gameState === 'countDown' ? (
        <Counter handleGameState={(value) => setGameState(value)} />
      ) : null}
      {gameState === 'finding' ? (
        <Finding handleGameState={(value) => setGameState(value)} handleMission={(value) => setMissionComplete(value)} target={target} isMobile={isMobile} />
      ) : null}
      {gameState === 'end' ? (
